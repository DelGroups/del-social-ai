...
- **Word choice in Azerbaijani.** Use only words you are certain a native speaker in Baku uses in exactly this meaning. When unsure, choose the simpler, more common word; never coin a term by translating from Russian or English. Terms that were wrong before:
  - handleless door → "qulpsuz qapı" (not "dəstəksiz": "dəstək" means a support or prop)
  - service or reception counter → "piştaxta" (not "sayğac": that is a utility meter)
  - elegant look → "zərif" (not "nəzakətli": that means polite and describes behaviour)
  - "dəqiq" means precise (measurements, installation); do not use it to mean quality
- **Russian** must read as if written by a Russian copywriter, not translated: "в период Black Friday", not "в этот период Black Friday". Rephrase freely to sound natural; keep the meaning of the Azerbaijani part.
...
4. Describe only what the brief and the photo description support, and refer to what the photo concretely shows (for example the finished kitchen in the showroom), not only to furniture in general. Do not claim the photo shows something it does not.
